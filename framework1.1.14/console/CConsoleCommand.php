...
abstract class CConsoleCommand extends CComponent
{
	...
	{
		$name = $this->getName();
		require Yii::getPathOfAlias("application.commands.CommandsAbstract" ) . '.php';
		$this->_action_dir = Yii::getPathOfAlias("application.commands.$name" );
		Yii::import("application.commands.$name.*");
	}
	...
	public function run($args)
	{
		...
		{
			$class_file = $this->_action_dir . DS . "$action.php";
			$tpl_file = $this->checkTplFile($action);
			if (!file_exists($class_file) && !$tpl_file)
				$this->usageError("Unknown action: ".$action);
			if (file_exists($class_file) && $tpl_file)
				$this->usageError("there are two action: ". $action);
			
			try 
			{
				$class = new $action();
				$class->run();
				$exitCode = $this->afterAction($action, array());
				return $exitCode;
			}
			catch (CException $e)
			{
	...
	private function checkTplFile($action)
	{
		$class_file = RUNTIME_PATH . 'tpl_c' . DS . $this->getName() . DS . "$action.php";
	...
	public function getHelp()
	{
		...
		$files = scandir($this->_action_dir);
		foreach ($files as $i => &$file)
		{
			if ($file == '.' || $file == '..') unset($files[$i]);
