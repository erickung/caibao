...
class ActiveRecordServ
{
	...
	public function getDataByField($field, $value, $type, $order=array(), $count=true)
	{
		...
		$cond_types = array($field=>$type);
		
		return $this->getData($conds , $cond_types, $order, $count);
	}
	
	public function getData($conds , $cond_types = array(), $order=array(), $count=true)
	{
		$data = array('rnt'=>array(),'total'=>0);
	
		$criteria = $this->getConditionByParams($conds, $cond_types);
		if (!empty($order))
			$this->setSort($criteria, $order);
	
		$data['rnt'] = $this->active_record->findAll($criteria);
		$data['total'] = $this->getAdapter()->count($criteria);
